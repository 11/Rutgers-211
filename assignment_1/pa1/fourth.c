...
int extractNumber(char str[])
{
	...
	for(i = 2; str[i]>47 && str[i]<58 && i < MAX_BUFFER-1; i++, pos++)
	{
		num[pos] = str[i];
	}
	num[pos] = '\0';
	...
	while((fgets(str,MAX_BUFFER,file))!=NULL)
	{
		...
		if(properFormat == true)
		{
			...
			if(str[0] =='i')
			{
				...
				if(lazyHashTable[choosePosition(inLineValue, lazyHashTable)].data == inLineValue)
				{
					printf("duplicate\n");
					fastCheck = true;
				}
				...
				if(choosePosition(inLineValue, lazyHashTable) != -1 && fastCheck == false)
				{
